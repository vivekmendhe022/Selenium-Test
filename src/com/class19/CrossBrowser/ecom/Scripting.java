...
public class Scripting {
	...
	void login() throws InterruptedException {
		...
		Assert.assertEquals(d.getTitle(), expectedLoginPageTitle, "Error: URL is not entered properly");
		...
		Assert.assertEquals(usernameTextField.getAttribute("value"), username, "Error: username is not entered properly");
		...
		Assert.assertEquals(passwordTextField.getAttribute("value"), password, "Error: password is entered not properly");
		Reporter.log("password is entered", true);
		
		// Click on login button
		d.findElement(By.id("loginButton")).click();
		Thread.sleep(5000);
		Assert.assertEquals(d.getTitle(), expectedHomePageTitle, "Error: Home page is not display.");
